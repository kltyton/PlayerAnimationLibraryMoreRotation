...
import com.kltyton.playeranimationlibrarymorerotation.mixin.CubePositionInvokerMixin;
...
public final class PalMoreBendableCuboids {
    ...
    public static void applyVectorBend(ModelPart part, PalMoreBendHolder bend) {
        ...
        // Bendable Cuboids 1.0.5 reapplies native bend every frame, including unchanged values.
    ...
    private static void applyExtendedBend(BendableCube cube, BendVectorState state) {
        if (!(cube instanceof CubePositionInvokerMixin cuboid)) {
            cube.applyBend(state.bendX());
            return;
        }

        Function<Vector3f, Vector3f> nativeBend = BendUtil.getBend(cube, state.bendX()).consumer();
        ...
        cuboid.palMore$iteratePositions(original -> {
    ...
    private static void applyNativeBendDirect(BendableCube cube, float bendX) {
        if (cube instanceof CubePositionInvokerMixin cuboid) {
            cuboid.palMore$iteratePositions(BendUtil.getBend(cube, bendX).consumer());
        } else {
