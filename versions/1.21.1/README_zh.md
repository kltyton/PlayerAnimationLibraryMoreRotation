...
- NeoForge: `21.1.250`
- Player Animation Library Fabric / Neo: `1.1.6+mc.1.21.1`
- bendable-cuboids: `2.0.0+alpha.1`
- Java: `21`
...
- 增加带 controller 的播放 API，并为 PAL 的 `timeline`、`sound_effects`、
  `particle_effects` 关键帧提供 controller-local handler。

## Gradle 依赖

发布到 Maven Local 的 artifact id 包含加载器名称和 Minecraft 版本：
...
dependencies {
    modImplementation "com.kltyton:playeranimationlibrarymorerotation-fabric-1.21.1:1.1.0"
    // 或：
    implementation "com.kltyton:playeranimationlibrarymorerotation-neoforge-1.21.1:1.1.0"
}
```
...
ResourceLocation animation = ResourceLocation.fromNamespaceAndPath("mob_battle", "poison_knife_animation");
...
```java
ResourceLocation attackController = ResourceLocation.fromNamespaceAndPath("mob_battle", "attack_controller");

PalMoreAnimations.play((ServerPlayer) player, animation, attackController);
```

服务端只同步 controller id。Java lambda 是本地 JVM 状态，不会被网络发送；
如果需要远端客户端执行 handler，必须先在客户端初始化阶段注册同一个 controller id。
...
PalMoreClientAnimations.playLocal(
        avatar,
        ResourceLocation.fromNamespaceAndPath("mob_battle", "poison_knife_animation")
);
...
PAL 原生已经会解析 Blockbench / Bedrock 的效果轨道：
...
`sound_effects` 和 `particle_effects` 也由 PAL 原生解析。本库提供的是
controller-local handler，把这些 PAL 原生关键帧路由给下游模组处理：
...
public static final ResourceLocation ATTACK_CONTROLLER =
        ResourceLocation.fromNamespaceAndPath("mob_battle", "attack_controller");
